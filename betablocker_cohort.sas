...
libname freq "/scratch/yf31/uhc/kl/freq";


data uhc.betablocker_cohort;
set uhc.med_phar_dat;
enroll = 0;
IF SUBSTR(DIAG1,1,3) in('413','428','404','414','402','401') 
or SUBSTR(DIAG1,1,4) in('4130','4139','4131','4289','4281','4282','4283','4284','4273', '4292') 
or SUBSTR(DIAG1,1,5) in('42731', '42732') THEN enroll = 1;
if enroll ne 0;
keep *;
run;
...
proc sort data=uhc.betablocker_cohort; by patid; run;
proc sort data=uhc.treatment; by patid; run;

data uhc.t_c;
merge uhc.betablocker_cohort uhc.treatment;
by patid;
run;
...
data uhc.t_c;
set uhc.t_c;
where (12 >= duration_1 >= -12) or fst_dt = .;
...
outcome8 = 0;
F SUBSTR(DIAG1,1,3) in('578', '430', '431', '432', '853') or SUBSTR(DIAG1,1,4) in('5310', '5312', '5314', '5316', '5320', '5322', '5324', '5326', '5330', '5332', '5334', '5336', '5340', '5342', '5344', '5346', '5693', '8520', '8521', '9582', '7847', '7848', '7863', '5997', '2871', '2872', '4590') or SUBSTR(DIAG1,1,5) in('53501', '53511', '53521', '53531', '53541', '53551', '53561', '53571', '53783', '53784', '56202', '56203', '56985', '56986', '56212', '56213') THEN outcome8 = 1;
drop start finish fst_dt lst_dt chk_dt fill_dt DAYS_SUP duration_1 duration_2 a b i BRND_NM;
run;

proc sql;
create table uhc.t_c_1(drop=rn) as
...
proc sql;
create table uhc.betablocker_merged as
